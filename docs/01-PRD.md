...
| ~~OQ-9~~ | ~~Which existing data must be migrated in (products, IMEIs, customer dues, supplier dues), and in what format?~~ **ANSWERED (M11): the format stopped mattering, and the plan is written.** The importer is mapping-driven — it reads CSV or `.xlsx`, guesses the columns from the words people actually type, and lets any guess be corrected — so no file layout had to be agreed in advance. What remained was the order, and it is fixed by the fact that imports run through the same services as manual entry: nothing can reference what has not arrived yet. Catalogue → products → customers and suppliers → handsets → opening stock → opening cash → dues, with a check after each step, in **docs/04 §10**. Deliberately *not* migrated: past sales, purchases and payments — ECITY starts from balances, because a re-keyed invoice is not the invoice that was issued and a half-migrated history would look complete while being wrong. Two things still to settle with the shop, both scheduling rather than design: the cut-off date, and who checks each step. | — | **Closed** |
