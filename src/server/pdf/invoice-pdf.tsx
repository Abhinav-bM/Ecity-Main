...
export function InvoiceDocument({ data }: { data: InvoiceData }) {
  ...
  for (const i of data.items) {
    const b = taxByRate.get(i.taxRateBasisPoints) ?? {
      cgst: 0n,
      sgst: 0n,
      igst: 0n,
    };
  ...
  return (
    ...
    >
      <Page size="A4" style={s.page}>
        <View style={s.header}>
          {/* An unregistered dealer must not head the document "Tax Invoice". */}
          <Text style={{ fontSize: 7.5, color: "#6b7280", letterSpacing: 1 }}>
            {data.gstEnabled ? "TAX INVOICE" : "INVOICE"}
          </Text>
          <Text style={s.shopName}>{data.business.name}</Text>
          {data.business.addressLine1 ? (
            <Text>{data.business.addressLine1}</Text>
          ) : null}
          <Text style={s.muted}>
        ...
        <View style={s.tableHead} fixed>
          ...
          {data.gstEnabled ? <Text style={s.cHsn}>HSN</Text> : null}
          <Text style={s.cQty}>QTY</Text>
          <Text style={s.cPrice}>PRICE</Text>
          {data.gstEnabled ? <Text style={s.cTax}>TAX</Text> : null}
          ...
          <View key={i.id} style={s.row} wrap={false}>
            <View style={s.cItem}>
              ...
              {i.identifierSnapshot ? (
                <Text style={[s.mono, s.muted, { fontSize: 7.5 }]}>
                  {i.identifierSnapshot}
                </Text>
              ) : null}
              {i.mainTypeSnapshot ? (
